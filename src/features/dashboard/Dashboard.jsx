...
const Dashboard = () => {
  ...
  const totalRevenue = Array.isArray(bookings) ? bookings.reduce((sum, b) => b.paymentStatus === 'Paid' ? sum + (b.amount || b.total || 0) : sum, 0) : 0;
        ...
        {stats.map((stat, idx) => (
          <div key={idx} className="premium-card p-8 border-none relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 ${stat.color === 'blue' ? 'bg-blue-50 text-blue-600 shadow-blue-500/10' :
