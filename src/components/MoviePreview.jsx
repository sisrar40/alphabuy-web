  ...
  return (
    <div className="bg-white rounded-[48px] shadow-soft border border-gray-50 flex flex-col lg:flex-row p-8 gap-12 group transition-standard hover:shadow-premium relative overflow-hidden">
      ...
      <div className="lg:w-80 w-full h-[480px] lg:h-[400px] relative flex-shrink-0 z-10">
        ...
        {trailerLabel && (
          <button className="absolute inset-x-8 bottom-8 flex cursor-pointer items-center justify-center bg-white/10 backdrop-blur-xl border border-white/20 py-4 rounded-2xl text-white text-sm font-bold uppercase tracking-widest hover:bg-white/20 transition-all opacity-0 group-hover:opacity-100">
        ...
        <div className="absolute top-6 left-6 flex flex-col gap-2">
           <span className="bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-bold text-gray-900 uppercase tracking-widest shadow-sm">UA 13+</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-8 justify-center z-10">
        <div className="space-y-4">
          <div className="flex gap-2 mb-2">
            {genres.slice(0, 2).map((g, i) => (
              <span key={i} className="text-[10px] font-bold text-aqua-600 uppercase tracking-widest bg-aqua-50 px-3 py-1 rounded-lg">{g}</span>
            ))}
          </div>
          <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight tracking-tight font-display">
            {title}
          </h2>
        ...
        <div className="flex flex-wrap gap-2">
          {formatTags?.slice(0, 4).map((format, index) => (
            <span
              key={index}
              className="px-4 py-2 text-[10px] bg-gray-50 text-gray-500 border border-gray-100 rounded-xl font-bold uppercase tracking-widest"
            >
              {format}
            </span>
          ))}
          ...
          {rating && (
            <div className="flex items-center gap-4 bg-gray-50/50 rounded-3xl px-6 py-4 border border-gray-100 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-[9px] uppercase tracking-wider text-gray-400 font-bold">{rating.votes} Explorers</p>
                </div>
            </div>
          )}
          ...
          <div className="flex gap-3">
             <button
                ...
                className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-standard ${
                  isFavorite ? "bg-red-500 text-white shadow-lg shadow-red-500/20" : "bg-white border border-gray-100 text-gray-400 hover:text-red-500 hover:border-red-100"
                }`}
              >
                <FaHeart className="text-xl" />
              </button>
              <button className="w-14 h-14 bg-white border border-gray-100 rounded-2xl flex items-center justify-center text-gray-400 hover:text-aqua-500 hover:border-aqua-100 transition-standard">
        ...
        <div className="pt-4 border-t border-gray-50 flex flex-wrap items-center justify-between gap-6">
           <div>
             <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Coming Next</p>
             <p className="text-sm font-bold text-gray-700">{releaseDate}</p>
           </div>
