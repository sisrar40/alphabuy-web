...
const MealSelectionStep = ({ nextStep, prevStep }) => {
  const dispatch = useDispatch();
  const {
    meals: reduxMeals,
    selectedAddOns: reduxAddOns,
    availableMeals,
    availableAddOns
  } = useSelector(
    (state) => state.booking,
  ...
  const mealPlans = (MEAL_PLANS || []).map((m) => ({
    ...m,
    popular: m.id === "lunch" || m.id === "family",
    icon:
      m.id === "breakfast" ? (
        <FaCoffee className="text-amber-600" />
      ) : m.id === "lunch" ? (
        <FaHamburger className="text-red-600" />
      ) : m.id === "dinner" ? (
        <GiMeal className="text-purple-600" />
      ) : m.id === "snacks" ? (
        <GiPopcorn className="text-yellow-600" />
      ) : m.id === "family" ? (
        <GiCakeSlice className="text-pink-600" />
      ) : (
          ...
          : m.id === "dinner"
            ...
            : m.id === "snacks"
              ...
              : m.id === "family"
                ? "pink"
                : "amber",
    features: m.benefits && m.benefits.length > 0 ? m.benefits : (
      m.id === "breakfast"
        ? ["Buffet", "Fruits", "Coffee", "Pastries", "Omelette"]
        : m.id === "lunch"
          ? ["Pasta", "Soft Drinks", "Salad", "Indian", "Asian"]
          : m.id === "dinner"
            ? ["Grill", "International", "Desserts", "Mocktails", "Premium"]
            : m.id === "snacks"
              ? ["Unlimited", "Ice Cream", "Juices", "Popcorn", "Nachos"]
              : m.id === "family"
                ? ["Portion Meals", "Kids Menu", "Cake", "Drinks", "Priority"]
                : [
                  "5-Course",
                  "Chef's Special",
                  "Wine/Beer",
  ...
  const [selectedAddOns, setSelectedAddOns] = useState(reduxAddOns || []);
  ...
  const toggleMeal = (mealId) => {
    ...
    dispatch(setBookingMeals(updatedMeals));
  ...
  const toggleAddOn = (addOnId) => {
    ...
    dispatch(setBookingAddOns(updatedAddOns));
    dispatch(calculateBookingTotal());
  };
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${mealPreferences[pref.id]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {mealPlans.map((meal) => {
            ...
            return (
              <div
                key={meal.id}
                className={`group relative bg-gradient-to-br ${getBgColorClass(meal.bgColor)} rounded-3xl overflow-hidden transition-all duration-300 cursor-pointer ${isSelected
              ...
              >
                ...
                <div className="p-6">
                  ...
                  {/* Selection Indicator */}
                  <div
                    className={`text-center py-3 rounded-xl text-xs font-bold transition-all ${isSelected
